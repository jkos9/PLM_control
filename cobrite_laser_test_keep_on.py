...
"""
deviceType = "DX1" 
keepRunningUntilTerminated = True
...
def run_demo():
    ...
    try:
        ...
        if keepRunningUntilTerminated:
            commands = [cmd for cmd in commands if cmd.strip().lower() != "stat 1,1,1,0"]
        # run all commands
        ...
        if keepRunningUntilTerminated:
            print("Laser stays on. Press Ctrl+C to terminate and turn it off.")
            while True:
                sleep(1)
    except KeyboardInterrupt:
        print("Termination requested. Turning laser off...")
        if idp_device is not None:
            try:
                idp_device.sendScpi("stat 1,1,1,0")
            except Exception as shutdown_error:
                print(f"Could not send shutdown command: {shutdown_error}")
    except Exception as e:
